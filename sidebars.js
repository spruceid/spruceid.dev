module.exports = {
  docs: {
    Introduction: ["primer"],
    "Developer Portal": ["devhome", "contributing", "code-of-conduct"],
    DIDKit: [
      "didkit/intro",
      "didkit/terminology",
      "didkit/concepts",
      "didkit/specs_and_deps",
      "didkit/rust",
      "didkit/install",
      "didkit/cli_commands",
      "didkit/http_syntax",
      "didkit/ffis",
      "didkit/changelog",
    ],
    "DIDKit Examples": [
      "didkit/examples",
      "didkit/example--core-functions-in-bash",
      "didkit/example--core-functions-in-curl",
      "didkit/example--batch-generation",
      "didkit/example--java-springboot",
      "didkit/example--svelte-chapi",
    ],
    Credible: [
      "credible/intro",
      "credible/terminology",
      "credible/concepts",
      "credible/install",
      "credible/authn",
    ],
    "Credible Examples": ["credible/examples", "credible/example--qr-issuance"],
    References: ["glossary", "further-reading"],
  },
};
